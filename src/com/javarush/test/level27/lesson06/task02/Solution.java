...
public class Solution {
    ...
    public static boolean isNormalLockOrder(final Solution solution, final Object o1, final Object o2) throws Exception {
        ...
        if(o1.hashCode() > o2.hashCode())
        {
            System.out.println(Thread.currentThread() + " - идет первым.");
        }
        else
        {
            Thread.sleep(200);
            System.out.println(Thread.currentThread() + " - идет c задежкой.");
        }
        ...
        synchronized (o1)
        {
            ...
            Thread.sleep(50);
            if (threadMethod.getState() == Thread.State.BLOCKED)
            {
                System.out.println(Thread.currentThread() + " - Совпадает.");
                threadMethod.interrupt();
                return true;
            }
            if (threadMethod.getState() == Thread.State.TIMED_WAITING)
            {
                System.out.println(Thread.currentThread() + " - Не совпадает.");
                threadMethod.interrupt();
                return false;
            }
        }
        System.out.println(Thread.currentThread() + " - завершил работу. Порядок неизвестен.");
