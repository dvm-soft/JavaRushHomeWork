...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class < T > outClass, Class ... addInterfaceClass)
    {
        List<Class> list = new ArrayList<>();
        list.addAll(Arrays.asList(addInterfaceClass));
        if (!list.contains(outClass))
        {
            list.add(outClass);
            addInterfaceClass = new Class[list.size()];
            for (int i = 0; i < list.size(); i++)
            {
                addInterfaceClass[i] = list.get(i);
            }
        }
        ItemInvocationHandler itemInvocationHandler = new ItemInvocationHandler();
        return (T) Proxy.newProxyInstance(outClass.getClassLoader(), addInterfaceClass, itemInvocationHandler);
