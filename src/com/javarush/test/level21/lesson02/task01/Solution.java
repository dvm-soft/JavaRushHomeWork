...
//        byte[] mask = new byte[]{(byte) 255, (byte) 255, (byte) 254, 0};
        byte[] ip = new byte[]{(byte) 192, (byte) 168,(byte) 172, (byte)177};
        byte[] mask = new byte[]{(byte) 255, (byte) 255, (byte) 255,(byte) 252};
    ...
    public static void print(byte[] bytes) {
        String result = "";
        for (int i = 0; i < 4; i++)
            result += Integer.toBinaryString(256+bytes[i]).substring(Integer.toBinaryString(256+bytes[i]).length()-8)
                    + (i == 3 ? "" : " ");
